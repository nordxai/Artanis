...
from ..exceptions import ArtanisException, HandlerError, MiddlewareError
...
class ExceptionHandlerMiddleware:
    ...
    def _format_error_response(self, exception: Exception, request: Any) -> Dict[str, Any]:
    ...
    def _log_exception(self, exception: Exception, request: Any) -> None:
        ...
        """
        method = getattr(request.scope, 'method', 'UNKNOWN') if hasattr(request, 'scope') else 'UNKNOWN'
        path = getattr(request.scope, 'path', 'UNKNOWN') if hasattr(request, 'scope') else 'UNKNOWN'
    ...
    async def __call__(self, request: Any, response: Response, next_middleware: Callable[..., Awaitable[Response]]) -> Response:
        ...
        try:
            ...
            if exception_type in self.custom_handlers:
                try:
                    return await self.custom_handlers[exception_type](e, request, response)
                except Exception as handler_error:
                    # If custom handler fails, fall back to default handling
                    self.logger.error(f"Custom exception handler failed: {str(handler_error)}")
                    e = handler_error
...
class ValidationMiddleware:
    ...
    async def __call__(self, request: Any, response: Response, next_middleware: Callable[..., Awaitable[Response]]) -> Response:
        ...
        if (self.validate_json and 
            hasattr(request, 'scope') and 
            request.scope.get('method') in ['POST', 'PUT', 'PATCH']):
                        ...
                        for field, validator in self.custom_validators.items():
                            if field in json_data:
                                try:
                                    ...
                                    raise ValidationError(
                                        message=f"Validator error for field {field}: {str(e)}",
                                        field=field,
